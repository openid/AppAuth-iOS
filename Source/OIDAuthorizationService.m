...
- (BOOL)shouldHandleURL:(NSURL *)URL {
	return [URL.absoluteString containsString:@"id_token"]
	&& [URL.absoluteString containsString:@"state"]
	&& [URL.absoluteString containsString:@"scope"];
}
...
+ (void)discoverServiceConfigurationForDiscoveryURL:(NSURL *)discoveryURL
																					 delegate:(id<NSURLSessionDelegate>)delegate
																				 completion:(OIDDiscoveryCallback)completion {
	NSURLSession *session = [NSURLSession sessionWithConfiguration:[NSURLSession sharedSession].configuration delegate:delegate delegateQueue:nil];
	NSURLSessionDataTask *task = [session dataTaskWithURL:discoveryURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
