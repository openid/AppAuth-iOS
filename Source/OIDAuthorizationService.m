...
+ (void)performRegistrationRequest:(OIDRegistrationRequest *)request callback:(OIDRegistrationCallback)callback {
  ...
  if (!URLRequest) {
    // A problem occurred deserializing the response/JSON.
    NSError *returnedError =
            [OIDErrorUtilities errorWithCode:OIDErrorCodeJSONSerializationError
                             underlyingError:nil
                                 description:@"The registration request could not be serialized as JSON."];
    dispatch_async(dispatch_get_main_queue(), ^{
        callback(nil, returnedError);
    });
    return;
  ...
  [[session dataTaskWithRequest:URLRequest
    ...
    OIDRegistrationResponse *registrationResponse =
            [[OIDRegistrationResponse alloc] initWithRequest:request parameters:json];
    ...
    // Success
    dispatch_async(dispatch_get_main_queue(), ^{
        callback(registrationResponse, nil);
    });
  }] resume];
}
