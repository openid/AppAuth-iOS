...
/*! @var kClientRegistrationAccessTokenTestValue
...
/*! @var kReigstrationClientURITestValue
...
/*! @var kTokenEndpointAuthMethodTestValue
...
+ (OIDRegistrationResponse *)testInstance {
  ...
  OIDRegistrationResponse *response =
...
- (void)testCopying {
  ...
  XCTAssertEqualObjects(response.clientIDIssuedAt, [NSDate dateWithTimeIntervalSince1970:kClientIDIssuedAtTestValue]);
  XCTAssertEqualObjects(response.clientSecret, kClientSecretTestValue);
  XCTAssertEqualObjects(response.clientSecretExpiresAt, [NSDate dateWithTimeIntervalSince1970:kClientSecretExpiresAtTestValue]);
  ...
  XCTAssertEqualObjects(response.tokenEndpointAuthenticationMethod, kTokenEndpointAuthMethodTestValue);
...
- (void)testSecureCoding {
  ...
  XCTAssertEqualObjects(responseCopy.registrationAccessToken, response.registrationAccessToken);
  XCTAssertEqualObjects(responseCopy.registrationClientURI, response.registrationClientURI);
  XCTAssertEqualObjects(responseCopy.tokenEndpointAuthenticationMethod, response.tokenEndpointAuthenticationMethod);
  XCTAssertEqualObjects(responseCopy.additionalParameters[kTestAdditionalParameterKey],
          kTestAdditionalParameterValue);
}

/*! @fn testMissingClientSecretWithClientSecretExpiresAt
 * @brief Make sure the registration response is verified to ensure the 'client_secret_expires_at' parameter exists
 * if a 'client_secret' is issued.
...
/*! @fn testMissingRegistrationAccessTokenWithRegistrationClientURI
...
/*! @fn testMissingRegistrationClientURIWithRegistrationAccessToken
